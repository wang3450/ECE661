...
if __name__ == "__main__":
    ...
    count = 0
    for y in range(card_image.shape[0]):
        for x in range(card_image.shape[1]):
            try:
                count += 1

                newPoint = homogenizePoints(H_inverse, (x,y))
                color = car_image[newPoint[1],newPoint[0]]
                card_image[y,x] = color
            except (IndexError):
                pass
    print(count)
