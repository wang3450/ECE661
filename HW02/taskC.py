...
def drawBoundingBox(PQRS, img):
    img = cv2.line(img, (PQRS[0][0], PQRS[0][1]),(PQRS[1][0], PQRS[1][1]), (255,255,255), 2)
    img = cv2.line(img, (PQRS[2][0], PQRS[2][1]), (PQRS[1][0], PQRS[1][1]), (255,255,255), 2)
    img = cv2.line(img, (PQRS[2][0], PQRS[2][1]), (PQRS[3][0], PQRS[3][1]), (255,255,255), 2)
    img = cv2.line(img, (PQRS[0][0], PQRS[0][1]), (PQRS[3][0], PQRS[3][1]), (255,255,255), 2)
    return img
...
if __name__ == "__main__":
    ...
    X = np.array([[556,1090], [556,2070], [4030,2070], [4030,1090]])      # X is a 4x2 ndarray of ROI on the rb18
    tv_image = getCardImage(int(sys.argv[1]))                 # card_image stores the cv2 image pointer of the cardImage
    car_image = cv2.imread('w13.jpg', cv2.IMREAD_UNCHANGED)     # car_image stores the cv2 image pointer of the rb18
    ...
    for key in tqdm(HOMO_ROI):
        try:
            tv_image[key] = HOMO_ROI[key]
        except(IndexError):
            pass

    '''Write the image to the file'''
    filename = 'tv{num}_car.jpeg'.format(num=sys.argv[1])
    cv2.imwrite(filename,tv_image)

    '''display image test code'''
    # cv2.imshow("test Car Image", drawBoundingBox(X, car_image))
    # cv2.waitKey(0)
